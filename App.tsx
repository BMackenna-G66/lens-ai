...
type Suite = 'compliance' | 'criminal' | 'admin' | 'general-dashboard' | 'regcheq' | 'customer-report' | null;
...
const AppContent: React.FC = () => {
  ...
  // 6b. Customer Report suite
  if (activeSuite === 'customer-report') {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-800 bg-slate-900/80 backdrop-blur">
          <button
            onClick={() => setActiveSuite(null)}
            className="flex items-center gap-2 text-slate-400 hover:text-white transition-colors text-sm font-semibold bg-slate-800 border border-slate-700 rounded-xl px-3 py-2"
          >
          ...
          <h1 className="text-white font-bold text-lg">Reporte de Cliente — PDF</h1>
        </div>
        <iframe
          src="http://localhost:10101"
