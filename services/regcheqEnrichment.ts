...
export async function fetchRegcheqEnrichment(rut: string, nombre = ''): Promise<RegcheqEnrichment> {
  ...
  try {
    ...
    // 2) Si le falta el SII, asegurar que la ficha exista y disparar la consulta
    //    tributaria vía el Worker (la external-api no expone ese trigger; la
    //    creación sola NO gatilla el SII). Fallback: reintento pasivo si no hay
    //    Worker configurado o si el disparo falla.
    if (!tieneSii(perfil)) {
      ...
      // 2b) Disparar el SII vía Worker usando el _id de la ficha como fichaId.
      ...
      // 2c) Fallback: si aún no hay SII (sin Worker o falló), reintento pasivo.
      if (!tieneSii(perfil)) {
        if (!perfil) await createRecord(rutN, nombre);
